...
export function BetCard({ bet, onVote }: BetCardProps) {
  ...
  let postedText = ""
  if (diffDays > 0) {
    postedText = `${diffDays}d ago`
  } else if (diffHours > 0) {
    postedText = `${diffHours}h ago`
  } else {
    postedText = "just now"
  }
          ...
          <div className="flex gap-3 sm:gap-4 p-3 sm:p-5">
            ...
            <div className="flex-1 flex flex-col min-w-0">
              <div className="flex items-center gap-1.5 mb-1.5">
                ...
                <span className="text-xs text-muted-foreground/60">• Posted {postedText}</span>
              </div>
          ...
          <div className="px-3 pb-3 sm:px-5 sm:pb-5 space-y-2 sm:space-y-3">
            <div className="space-y-2">
              <div className="flex gap-1 h-2.5 rounded-full overflow-hidden bg-muted shadow-inner">
                <div
                  className="bg-gradient-to-r from-emerald-100 to-emerald-200 dark:from-emerald-900/40 dark:to-emerald-800/40 transition-all duration-500"
                  style={{ width: `${yesPct}%` }}
                />
                <div className="bg-gradient-to-r from-rose-100 to-rose-200 dark:from-rose-900/40 dark:to-rose-800/40 flex-1 transition-all duration-500" />
              </div>
              <div className="flex justify-between text-xs">
                <span className="text-emerald-600 dark:text-emerald-400 font-semibold">Yes {yesPct}%</span>
                <span className="text-rose-600 dark:text-rose-400 font-semibold">No {100 - yesPct}%</span>
              </div>
            </div>
